...
- These 5 project documents (PRD, ARCHITECTURE, REQUIREMENTS, CONVENTIONS, SCOPE) are the source of truth.
- Update the relevant document when a decision changes.
- Do not create ad-hoc markdown files for planning or notes — keep decisions in the appropriate document. If there is a need for additional files that shouldn't live under the main 5 documents, ask first.
