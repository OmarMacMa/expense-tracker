...
> The current shipped version is tracked on [GitHub Releases](https://github.com/OmarMacMa/expense-tracker/releases). This document defines version **scope** (what each version contains), not what has shipped — the latest tag on `main` is the source of truth for that.
